-- Campaign-level daily performance (Activity / event-date), Pull-scaled.
--
-- MCP Unique users metrics are cumulative. Campaign-level Activity is scaled
-- to Pull media-source totals for correct absolute numbers.  The Activity
-- proportional distribution among campaigns within each media source is
-- preserved from MCP.
--
-- When Master API becomes available, replace with direct Activity data.

DROP VIEW IF EXISTS activity_campaign_daily_la;

CREATE VIEW activity_campaign_daily_la AS
WITH
pull_ms AS (
  SELECT
...
mcp_campaign AS (
  SELECT
    fact_date, media_source, campaign,
