-- Ad-level daily performance (LTV) from MCP, proportionally scaled to Pull
-- media-source totals (authoritative).  When Pull has no data for a
-- media_source on a given day, MCP ad values pass through unscaled.
-- Ad→Adset mapping joined from mcp_adset_ad_map.

DROP VIEW IF EXISTS growth_ad_daily_la;

CREATE VIEW growth_ad_daily_la AS
WITH
pull_ms AS (
  SELECT
...
SELECT
  ...
  a.currency,
  CASE WHEN p.spend IS NOT NULL THEN 'appsflyer_mcp_ad_scaled' ELSE 'appsflyer_mcp_ad_raw' END AS primary_source,
