-- Ad-level daily performance (Activity / event-date), Pull-scaled.
--
-- MCP Unique users metrics are cumulative. Ad-level Activity is scaled
-- to Pull media-source totals for correct absolute numbers.  The Activity
-- proportional distribution among ads is preserved from MCP.
-- Ad→Adset mapping joined from mcp_adset_ad_map.
--
-- When Master API becomes available, replace with direct Activity data.

DROP VIEW IF EXISTS activity_ad_daily_la;

CREATE VIEW activity_ad_daily_la AS
WITH
pull_ms AS (
  SELECT
...
mcp_ad_raw AS (
  SELECT
    ...
    COALESCE(af_start_trial, 0) AS af_start_trial,
    COALESCE(af_subscribe, 0) AS af_subscribe,
    COALESCE(rc_trial_converted_event, 0) AS rc_trial_converted_event,
    COALESCE(af_tutorial_completion, 0) AS af_tutorial_completion,
    currency,
    fetched_at
  FROM marketing_fact_ad
  WHERE attribution_model = 'activity'
),
mcp_ad_ms AS (
...
ad_with_adset AS (
  SELECT
    a.*,
    COALESCE(NULLIF(a.adset, ''), m.adset, '') AS resolved_adset
  FROM mcp_ad_raw a
  LEFT JOIN mcp_adset_ad_map m
    ON m.media_source = a.media_source AND m.campaign = a.campaign AND m.ad = a.ad
)
SELECT
  a.fact_date,
  ...
  a.currency,
  CASE WHEN p.spend IS NOT NULL THEN 'appsflyer_mcp_ad_activity_scaled' ELSE 'appsflyer_mcp_ad_activity_raw' END AS primary_source,
  CASE WHEN p.spend IS NOT NULL THEN 'high' ELSE 'medium' END AS source_confidence,
  a.fetched_at
FROM ad_with_adset a
LEFT JOIN mcp_ad_ms t
